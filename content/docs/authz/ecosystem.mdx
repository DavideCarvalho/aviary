...
## Telescope — `@dudousxd/nestjs-authz-telescope`

The [Telescope](/docs/telescope) integration adds an **authorization watcher**. Every decision — the ability, the allow/deny verdict, the reason, and the user — is recorded and surfaced in a dedicated tab. When a `403` shows up that you didn't expect, you stop guessing: the watcher shows exactly which policy method ran and why it denied.

<Callout type="info">
This is the single biggest quality-of-life win for debugging authorization. Authorization failures are notoriously opaque ("forbidden — but why?"); the watcher turns them into a readable trail.
</Callout>
