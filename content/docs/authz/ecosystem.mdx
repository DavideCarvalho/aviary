...
<Callout type="warn" title="Renamed in alpha">
The dedicated `@dudousxd/nestjs-authz-telescope` package is **retired** — its job is now done by the generic `@dudousxd/nestjs-diagnostics-telescope` watcher. The `AUTHZ_DECISION_CHANNEL` constant was likewise removed; the channel name is now derived through `@dudousxd/nestjs-diagnostics`.
</Callout>
