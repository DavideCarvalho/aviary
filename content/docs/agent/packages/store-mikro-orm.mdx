...
and no boot-time heal in the path, which is what keeps the SQLite caveat below out of it. The match
...
<Callout type="warn" title="ensureAgentSchema's add-column heal is dialect-dependent">
The heal applies `getUpdateSchemaSQL({ safe: true })` and keeps only the statements targeting an
agent table. On MySQL and Postgres a new column is a plain `alter table … add column`, so an existing
database picks it up on the next boot. On **SQLite/libsql** MikroORM expresses the same change as a
table rebuild (`create table agent_thread__temp_alter` → copy → `drop` → `rename`), and those
statements name a table the filter does not recognise, so they are dropped and the column appears on
a fresh database only. If you run the agent store on SQLite, add the column with
[`agentSchemaSql`](#exports) in a migration instead of relying on the boot heal.
</Callout>
