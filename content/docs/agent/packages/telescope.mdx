...
— the Agent tab's governance sections and the trace waterfall are additive, not a replacement. Since a turn's long steps are dispatched, the
`llm.turn`/`tool.execution` spans for a given run originate from whichever worker actually executed
that step, not necessarily the pod that started the run — the trace still stitches them together by `traceId = runId`.
</Callout>
