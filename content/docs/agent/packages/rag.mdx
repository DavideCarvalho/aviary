...
<Callout type="info" title="Custom vector stores">
  `VectorStore` (`upsert` / `search` / `remove`) is the extension point — implement it over Qdrant,
  Pinecone, Weaviate, or your own `-data` SQL seam, and pair it with any `EmbeddingProvider` via
  `EmbeddingRetriever`. (`remove(documentId)` deletes a document's chunks — needed for delete-sync and
  to avoid orphaning the tail when a re-ingest produces fewer chunks.)
</Callout>
