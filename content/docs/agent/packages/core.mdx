...
| `AgentStore` | ORM-agnostic persistence for threads, messages, tool calls, and usage. | `createThread` / `getThread` / `listThreads` / `forkThread` / `appendMessage` / `truncateFrom` / `recordToolCall` / `updateToolCall` / `recordUsage` / `quotaToday`. |
...
| `ActorResolver` | Resolves the caller for an inbound request — the identity seam; no default fabricates one. | `resolve(req: unknown): Actor \| Promise<Actor>`. |
...
(`ToolForbiddenError`, `ToolNotFoundError`, `ToolInputInvalidError`, `QuotaExceededError`), and the
`aviary:agent:*` diagnostics helpers — re-exported by the umbrella package too, so an app importing
`@dudousxd/nestjs-agent` never needs a direct dependency on `-core`.
