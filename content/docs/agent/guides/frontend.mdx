...
`renderReasoning` + `reasoningLabel` (the model's thinking, folded behind a toggle), `onFork` /
`editable` + `onEditSubmit` / `regeneratable` + `onRegenerate`, and per-message `getUsage` /
`getCreatedAt` resolvers for the cost + timestamp line. Pass `<MessageItem>` directly if you want to
lay the list out yourself.

Both components are thin renderers over `useChatTranscript` — the same model is exported, so a
different layout is a rewrite of the markup only, not of the part grouping, the edit machine, or the
windowing. See [Bring your own UI](/docs/agent/guides/bring-your-own-ui#the-transcript-model).

### Stopping a turn from the composer

`ChatInput` renders a cancel affordance next to send while a turn is in flight — it calls the same
`POST /agent/chat/:runId/cancel` that `chat.cancel()` does:
...
Without `onStop` nothing is rendered, so the affordance exists exactly when there is something to
cancel. (`ChatInput`'s dictation mic has its own "Stop" — the two never collide, the cancel button
defaults to "Stop generating".)
...
## Sub-agents the chat did not wait for

A [detached delegation](/docs/agent/guides/multi-agent#delegate-without-waiting) finishes after the
turn that started it, so its answer is not something the current stream will ever carry. Set
`background: true` and the hook tracks them:

```tsx
const chat = useAgentChat({ threadId, background: true });

{chat.background.isWorking && (
  <p>{chat.background.runs.filter((r) => r.status === 'running').length} agent(s) working…</p>
)}
```

| Field | What it is |
|---|---|
| `background.runs` | One `BackgroundRun` per delegation this thread started: `{ runId, agent, toolCallId, toolName, status, message? }` |
| `background.isWorking` | Whether any of them is still `running` |
| `background.refresh()` | Re-read now instead of waiting for the next poll |

When one lands, its message is appended to `chat.messages` — no reload, and no re-seeding of the
list, so a tool card mid-flight or a question set awaiting an answer is left alone.

Everything comes from **one thread read**, because both halves are the same fact persisted: the
receipt is a tool result on the delegating turn, and the answer is a message stamped with the run
that wrote it. So a reload, or a second tab, sees exactly what the tab that started it sees. The
interval (`backgroundPollMs`, default `5000`; `0` disables it) runs only while something is
outstanding — a conversation with nothing in flight costs no requests, and neither does one that
never opted in, which is why `background` is off by default.

<Callout type="info" title="Why a poll and not a second stream">
What a background run produces is a persisted **message**, not tokens this chat should render as they
arrive: it can land while the user is mid-turn or has the tab shut, and it has to be there on the next
read either way. Its own token stream is still subscribable on its own run id
(`GET /agent/chat/:runId/stream`) if you want to show it working.
</Callout>
...
- [Bring your own UI](/docs/agent/guides/bring-your-own-ui) — skip `MessageList`/`ChatInput` and render off `useAgentChat` + `useChatTranscript`, with a fully custom design system
