...
|---|---|---|
| Threads | `agent_thread` | `createThread`, `getThread`, `listThreads`, `softDeleteThread`, `forkThread`, `setTitle`, `setActiveStream`, `ownerOfThread` |
...
`ownerOfThread` and `ownerOfToolCall` resolve the owning `actorRef` (or `null` if the row doesn't
exist) — the governance endpoints (`/agent/threads/:id`, `/agent/tool-call/approve` · `/reject`) call
these to assert the acting actor owns the target before acting on it.
...
survives pnpm's dual ESM/CJS copies). A store module binds them **globally** (a Nest global module),
so any module in the app resolves them without re-importing. That's why `store` on
