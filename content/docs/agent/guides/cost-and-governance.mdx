...
<Tabs items={['Standalone dashboard', 'Telescope tab']}>
  ...
  <Tab value="Telescope tab">
    Already running `@dudousxd/nestjs-telescope`? Add the extension to get the same governance
    sections as an "Agent" tab (agent runs also show up in the durable "Workflows" tab for free):

    ```ts
    import { AgentTelescopeModule } from '@dudousxd/nestjs-agent-telescope';

    @Module({ imports: [AgentTelescopeModule.forRoot()] })
    export class AppModule {}
    ```
  </Tab>
</Tabs>
...
- [Persistence](/docs/agent/guides/persistence) — the stores that bind `AGENT_STORE` + `AGENT_GOVERNANCE_QUERIES`
