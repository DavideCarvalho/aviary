...
| `AGENT_QUOTA_STORE` | `QuotaStore \| undefined` | `options.quota` | You need to read/write token budgets outside `AgentService.quotaToday`. |
...
| `AGENT_ACTOR_RESOLVER` | `ActorResolver` | `options.actorResolver`, else `UnconfiguredActorResolver` (throws on every request) | You need to resolve the calling `Actor` yourself, e.g. in a guard. |
