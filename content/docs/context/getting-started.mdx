...
The factory returns the **same** `ContextModuleOptions` that `forRoot()` accepts — the trace-id hook, `initialize`, the middleware options, the cross-process `carrier`, `baggage`, and `enrichers` are all honoured. If you prefer a class, implement `ContextModuleOptionsFactory` and pass it as `useClass`:

```ts title="src/context-options.factory.ts"
...
```ts title="src/notifications.service.ts"
import { Injectable } from '@nestjs/common';
