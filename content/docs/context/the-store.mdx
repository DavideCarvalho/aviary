...
- **`traceparent`** — the parsed upstream `traceparent` (span-id + trace-flags), captured at request start so a re-emitted downstream `traceparent` faithfully continues the distributed trace. **Process-local on purpose** — intentionally *not* part of the default cross-process carrier. See [Cross-Process → faithful traceparent](/docs/context/cross-process#faithful-traceparent-propagation).
...
When you pass the captured upstream as the second argument — `toTraceparent(traceId, Context.get()?.traceparent)` — the re-emitted header continues the incoming trace faithfully (propagating the parent span-id and the upstream *sampled* flag) instead of minting a new one. See [Cross-Process → faithful traceparent](/docs/context/cross-process#faithful-traceparent-propagation).
