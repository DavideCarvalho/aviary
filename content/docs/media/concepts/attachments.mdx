...
Both models sit on top of the same [storage layer](/docs/media/concepts/two-layers#layer-1--storage) and share the same [image conversions](/docs/media/concepts/conversions). They differ only in **where the metadata lives**.
...
You don't construct attachments by hand; the manager uploads the bytes and hands you the value object. In a NestJS app it's `media.attachments` (always available — it only needs storage, and an [image processor](/docs/media/packages/image-sharp) if you want variants).
...
| ORM | How | Page |
|---|---|---|
| TypeORM | `@AttachmentColumn()` (or the `attachmentTransformer` on a `simple-json` column) | [database-typeorm](/docs/media/packages/database-typeorm#attachment-column) |
| MikroORM | `AttachmentType` custom type | [database-mikro-orm](/docs/media/packages/database-mikro-orm#attachment-column) |
| Prisma | `toAttachmentJson` / `fromAttachmentJson` on a `Json` field | [database-prisma](/docs/media/packages/database-prisma#attachment-column) |
| Drizzle | the `attachment()` custom column | [database-drizzle](/docs/media/packages/database-drizzle#attachment-column) |
