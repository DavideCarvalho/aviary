...
`publicUrls` gates `url()`, `presign` gates `temporaryUrl()` and is consulted by [`uploadMode`](/docs/media/concepts/uploads) to decide direct vs proxied uploads. A driver that can't do something throws `UnsupportedOperationError` instead of silently faking it.
...
| `conversions` | Image presets available for this collection (see [Conversions](/docs/media/concepts/conversions)) |
...
The `media` table is polymorphic and managed by your chosen [ORM store](/docs/media/concepts/persistence) — TypeORM, MikroORM, Drizzle, or Prisma, all behind the same `MediaStore` contract.
