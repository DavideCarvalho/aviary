...
`listByOwner` returns records ordered by `order` ascending; `nextOrder` returns the next append position in a collection. That's the whole surface the `MediaLibrary` needs — which is why a [custom store](/docs/media/recipes/custom-store) is easy.
...
| Adapter | Auto-schema |
|---|---|
| [TypeORM](/docs/media/packages/database-typeorm) | `ensureMediaSchema` — create table + add missing columns; never drop/alter. `autoCreateSchema: false` to opt out |
| [MikroORM](/docs/media/packages/database-mikro-orm) | `updateSchema({ safe: true })` — same guarantees, native |
| [Drizzle](/docs/media/packages/database-drizzle) | migration-first (drizzle-kit); `createMediaTable` helper for dev/test |
| [Prisma](/docs/media/packages/database-prisma) | consumer-managed — you own the schema + migrations |
