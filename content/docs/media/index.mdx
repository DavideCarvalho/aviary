...
- **Ecosystem glue points**: `nestjs:media:*` diagnostics channels, a [Telescope](/docs/media/telescope) watcher, a codegen-emitted typed client, and a React `useMediaUpload` hook + `<MediaUploader/>`.
...
You *don't* need the media-library layer for everything. Writing a generated report to disk, or caching a rendered asset, is pure [raw storage](/docs/media/recipes/raw-storage) — register a disk and skip the `store`. And if another library just needs a filesystem (mail attachments, a durable workflow step), it imports the facade through the subpath:
...
media is plugged into the same four glue points every `@dudousxd/nestjs-*` library shares — it emits to [diagnostics](/docs/media/diagnostics) channels that [Telescope](/docs/media/telescope) consumes, contributes a typed client through [Codegen](/docs/media/codegen), and ships a `-react` family package. See [Concepts → Diagnostics & the glue points](/docs/media/concepts/diagnostics) for the wiring.

Continue to [Getting Started](/docs/media/getting-started).
