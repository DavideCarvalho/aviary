...
- `presign + multipart` are what [`uploadMode: 'auto'`](/docs/media/concepts/uploads) reads to choose the **direct** (presigned multipart) upload path.
...
<Callout type="info">
The driver's behavior is validated by the shared conformance suite running against a real **MinIO** server (testcontainers) — not just mocks — so `put/get/stream/copy/move/size/delete` and error semantics match a real backend. See [`-testing`](/docs/media/packages/testing).
</Callout>
