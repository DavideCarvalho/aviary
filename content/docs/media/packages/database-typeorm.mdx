...
Beyond the `media` table, this package also wires the [attachment column model](/docs/media/concepts/attachments) — a single file stored as a JSON value object directly on your entity. Use the `@AttachmentColumn()` decorator:
...
`typeorm` (^0.3) and `@dudousxd/nestjs-media-core`. See [Concepts → Persistence](/docs/media/concepts/persistence) for the shared store contract, and [Concepts → Attachments](/docs/media/concepts/attachments) for the column model.
