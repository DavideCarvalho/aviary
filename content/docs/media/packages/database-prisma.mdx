...
Prisma has no entity classes, so the [attachment column model](/docs/media/concepts/attachments) is two null-safe helpers around a `Json` column instead of a decorator. Add a `Json?` field to your model:
...
`@dudousxd/nestjs-media-core` only — you bring your own `@prisma/client`. See [Concepts → Persistence](/docs/media/concepts/persistence) and [Concepts → Attachments](/docs/media/concepts/attachments).
