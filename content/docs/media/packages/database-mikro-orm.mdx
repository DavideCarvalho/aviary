...
For the [attachment column model](/docs/media/concepts/attachments) — a single file as a JSON value object on your entity — register the `AttachmentType` custom type on a property:
...
`@mikro-orm/core` (^6) and `@dudousxd/nestjs-media-core` — plus your driver package (`@mikro-orm/postgresql`, `@mikro-orm/mysql`, `@mikro-orm/better-sqlite`, …). See [Concepts → Persistence](/docs/media/concepts/persistence) and [Concepts → Attachments](/docs/media/concepts/attachments).
