...
For the [attachment column model](/docs/media/concepts/attachments) — a single file as a JSON value object on your own table — use the `attachment()` custom column:
...
`drizzle-orm` (^0.38) and `@dudousxd/nestjs-media-core`, plus a driver (`better-sqlite3`, `libsql`, …). See [Concepts → Persistence](/docs/media/concepts/persistence) and [Concepts → Attachments](/docs/media/concepts/attachments).
