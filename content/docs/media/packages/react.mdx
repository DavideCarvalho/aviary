...
The React glue point. A `useMediaUpload` hook and a drop-in `<MediaUploader/>`, both built on the resumable [`-client`](/docs/media/packages/client) — progress, status, and resume handled for you.
...
For convenience, the package re-exports `uploadMedia` and `mediaUrl` from [`-client`](/docs/media/packages/client), so a React app needs only this one install.
