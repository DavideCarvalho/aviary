...
Upload behavior also follows the disk: an S3 disk defaults to the [`direct`/presigned path](/docs/media/concepts/uploads), a local disk to `proxy` — each overridable via `uploadMode`.
