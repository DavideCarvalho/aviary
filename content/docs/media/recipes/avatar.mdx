...
An avatar is the textbook case for the [attachment column model](/docs/media/concepts/attachments) — one file, one field, no separate `media` table. The avatar *is* a column on your `User`:
...
No collection config, no `list`, no `ownerType`/`ownerId` — just a column. Reach for the collection above instead when a user can have *several* images or you want lazy (on-demand) conversions. See [Concepts → Attachments](/docs/media/concepts/attachments) for the full comparison and the per-ORM column wiring.
