...
The four bundled [ORM stores](/docs/media/concepts/persistence) cover the common cases, but the contract is small — back media records with anything (a different ORM, a document store, a service API) by implementing five methods.
...
For a real backend, also run it as a `*.db.spec.ts` against a container (the way the ORM stores are validated on Postgres). See [`-testing`](/docs/media/packages/testing) and [Persistence](/docs/media/concepts/persistence).
