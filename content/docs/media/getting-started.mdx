...
The `TypeOrmMediaStore` creates the `media` table on first use by default (non-destructive — see [Persistence](/docs/media/concepts/persistence)). Nothing else to migrate to get started.
...
- [Concepts → The two layers](/docs/media/concepts/two-layers) — the model in depth
- [Concepts → Uploads & multipart](/docs/media/concepts/uploads) — resumable tus + presigned direct
- [Concepts → Conversions](/docs/media/concepts/conversions) — lazy vs eager image variants
- [Recipes](/docs/media/recipes) — avatars, galleries, direct-to-S3, raw storage
- [Reference → Configuration](/docs/media/reference/configuration) — every `MediaModule` option
