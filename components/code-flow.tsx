...
export class EmailService {
  ...
  steps: [
    {
      lines: [11, 11],
      split: { file: 1, lines: [4, 5], window: [2, 6], hint: "reserve" },
      stage: "",
      active: 0,
      tone: "run",
      title: "reserve",
    ...
    {
      lines: [12, 12],
      split: { file: 1, lines: [10, 11], window: [8, 12], hint: "charge" },
      stage: "",
      active: 1,
      tone: "run",
      title: "charge",
    ...
    {
      lines: [14, 14],
      split: { file: 1, lines: [16, 17], window: [14, 18], hint: "ship" },
      stage: "",
      active: 3,
      tone: "run",
      title: "ship",
    ...
    {
      lines: [15, 15],
      split: { file: 1, lines: [22, 23], window: [20, 24], hint: "confirm" },
      stage: "",
      active: 4,
      tone: "run",
      title: "confirm",
...
export class KycWorkflow {
  ...
  steps: [
    ...
    },
    {
      file: 0,
      lines: [12, 12],
      split: { file: 1, lines: [2, 3], window: [1, 4], hint: "KycWorkflow" },
      stage: "",
      title: "ctx.child",
...
async approve(@Param('orderId') orderId: string, @Body() body: Approval) {
  ...
  steps: [
    ...
    },
    {
      ...
      caption:
        "waitForSignal parks the run on the token — zero compute, no worker held, for as long as it takes.",
    },
    {
      file: 0,
      lines: [8, 8],
      ...
      tone: "run",
      title: "signal →",
      actor: "workflows.signal('approved', body) — from outside",
      caption:
        "Someone approves: a controller (or webhook, or another service) sends the signal by token. Sent before anyone waits? It buffers — a signal is never lost.",
    },
    {
      file: 0,
      lines: [9, 9],
  ...
  steps: [
    {
      ...
      caption:
        "The schedule lives at the module level — cron fires and the engine starts the workflow it names with a deterministic per-window run id, so a double-fire of the same window never creates two runs.",
    },
    {
      file: 0,
      lines: [5, 5],
...
export class PipelineWorkflow {
  ...
  steps: [
    ...
    },
    {
      ...
      caption:
        "Crash-recovery reclaims the orphaned run and resumes it — it throws again, and again. Each pickup increments recoveryAttempts toward maxRecoveryAttempts.",
    ...
    },
    {
      ...
      stage: "",
      title: "DLQ: page",
      actor: "@DeadLetter handler starts as its own run",
      caption:
        "Dead-lettering starts the @DeadLetter method as a separate durable run (pipeline.dlq), idempotent by dlq:<runId> — the dead run stays parked where it is. The handler’s first step pages on-call.",
    ...
    },
    {
      ...
      stage: "",
      title: "ticket",
      actor: "ctx.step → open a ticket with the original input",
      caption:
        "A second step opens a ticket carrying the dead run’s original typed input — ready to replay once the bug is fixed.",
      child: { pActive: 3, cActive: 1, pTone: "fail" },
    },
    {
      lines: [20, 20],
...
export class PaymentsService {
  ...
  steps: [
    ...
    },
    {
      file: 0,
      lines: [11, 11],
      split: { file: 1, lines: [5, 5], window: [5, 8], hint: "chargeCard" },
      stage: "",
      active: 1,
      tone: "run",
      title: "retry policy",
    ...
    },
    {
      file: 0,
      lines: [11, 11],
      split: { file: 1, lines: [5, 5], window: [5, 8] },
      stage: "",
      active: 1,
      tone: "wait",
      title: "backoff",
  ...
  steps: [
    {
      ...
      tone: "run",
      title: "validate",
      actor: "ctx.step → validate (same NestJS app)",
      caption:
        "The in-process event-emitter transport dispatches this to a @Step handler in the SAME process — no network hop.",
    },
    {
      file: 0,
      lines: [10, 10],
      ...
      caption:
        "Identical ctx.step call — but this transport carries the dispatch to a different worker process over Redis/SQS. The workflow code never changes.",
    },
    {
      file: 0,
      lines: [11, 11],
  ...
  steps: [
    {
      file: 0,
      lines: [6, 8],
      split: { file: 1, lines: [9, 10], window: [9, 10], hint: "bookFlight" },
      stage: "",
      active: 0,
      tone: "run",
      title: "flight",
    ...
    },
    {
      file: 0,
      lines: [9, 11],
      split: { file: 1, lines: [12, 13], window: [12, 13], hint: "bookHotel" },
      stage: "",
      active: 1,
      tone: "run",
      title: "hotel",
    ...
    },
    {
      ...
      tone: "run",
      title: "undo hotel",
      actor: "engine dispatches cancelHotel — checkpoint −1",
      caption:
        "The engine walks the stack in reverse and DISPATCHES the compensate registered here — cancelHotel, below — to its worker like any durable step, checkpointed at reserved seq −1: a crash mid-unwind resumes here instead of re-running finished undos. It receives the { input, output } of the hotel booking it undoes.",
    },
    {
      file: 0,
      lines: [7, 7],
      ...
      tone: "run",
      title: "undo flight",
      actor: "cancelFlight({ input, output }) — checkpoint −2",
      caption:
        "Then the flight's compensate runs, with both the original input AND the booking it must cancel in its envelope — UndoOf<TripService['bookFlight']> types it for free, and a Python worker could serve it by name.",
    },
    {
      file: 0,
      lines: [12, 13],
...
export class SendReceiptWorkflow {
  ...
  steps: [
    ...
    },
    {
      file: 0,
      lines: [7, 7],
      split: { file: 1, lines: [5, 6], window: [4, 7], hint: "emails" },
      stage: "",
      active: 1,
      tone: "wait",
      title: "blocked",
      ...
      caption:
        "The queue this line names — registered in app.module.ts, below — is full (concurrency: 2), so admission is blocked. The engine does NOT dispatch — it re-suspends the run with the retry time as wakeAt, and the timer poller retries later. Zero compute held.",
    },
    {
      file: 0,
      lines: [8, 8],
    ...
    },
    {
      file: 0,
      lines: [9, 9],
      split: { file: 1, lines: [6, 6], window: [4, 7], hint: "fairnessKey" },
      stage: "",
      active: 3,
      tone: "wait",
      title: "fair share",
      ...
      caption:
        "Within the same priority tier, fairness breaks the tie: with the queue's fairness: 'key' set (below), the least-recently-served fairnessKey is admitted next, so one busy tenant can't starve the others.",
    },
    {
      file: 0,
      lines: [6, 11],
  ...
  steps: [
    {
      ...
      caption:
        "The singleton key derives store:A from the input. The first start (below) finds the key’s only slot free (limit defaults to 1 — a mutex) and is admitted.",
      child: { pActive: 0, cActive: -1, pTone: "run" },
    },
    {
      file: 0,
    ...
    },
    {
      ...
      caption:
        "A second start for store:A arrives while run 1 is in flight. It is a real run with its own runId — but the key function derives the SAME store:A.",
    ...
    },
    {
      ...
      caption:
        "The admission gate counts run 1 under the same key, so run 2 is NOT admitted: it suspends with a jittered retry and a wake-on-release notify — zero compute while it queues. store:B (last line below) has its own key and runs immediately.",
    ...
    },
    {
      ...
      caption:
        "Run 2 is admitted and does its own sync — exactly one store:A sync at a time, and none of the requests were lost.",
      child: {
        pActive: 2,
        pDone: true,
        pTone: "done",
    ...
    },
    {
      ...
      caption:
        "Run 2 completes. Set maxQueueDepth to bound how many starts may queue behind the slot — past it, start() rejects with SingletonQueueFullError instead of growing the backlog.",
      child: {
        pActive: 2,
        pDone: true,
        pTone: "done",
